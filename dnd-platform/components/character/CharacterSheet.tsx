              ...
              <div>
                ...
                <div className="grid grid-cols-6 gap-3">
                  {Object.entries(character.ability_scores).map(([ability, score]) => (
                    <div key={ability} className="bg-white/5 dark:bg-black/20 rounded-xl p-3 text-center">
                      <p className="text-xs text-gray-400 uppercase font-medium">{ability}</p>
                      <p className="text-2xl font-bold text-white my-1">{score}</p>
                      <p className="text-sm text-gray-400">{mod(score)}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-3">
                  {[
                    ...
                    <div key={label} className="bg-white/5 dark:bg-black/20 rounded-xl p-3 text-center">
                  ...
                  {Object.entries(character.saving_throws).map(([ability, proficient]) => {
                    ...
                    return (
                      <div key={ability} className="flex items-center gap-2 text-sm">
                        <span className={`w-2 h-2 rounded-full ${proficient ? 'bg-amber-highlight' : 'bg-white/10'}`} />
          ...
          {tab === 'skills' && (
            <div className="space-y-1">
              {SKILLS.map(({ name, ability }) => {
                ...
                const total = proficient ? base + character.proficiency_bonus : base
                return (
                  <div key={name} className="flex items-center gap-3 py-1.5 px-2 rounded-lg hover:bg-white/5">
                    <span className={`w-2 h-2 rounded-full flex-shrink-0 ${proficient ? 'bg-amber-highlight' : 'bg-white/10'}`} />
                    <span className="flex-1 text-sm text-gray-300">{name}</span>
                    <span className="text-xs text-gray-400 uppercase">{ability}</span>
                    <span className="font-medium text-white w-8 text-right">
                      {total >= 0 ? '+' : ''}{total}
                    </span>
                  </div>
                )
